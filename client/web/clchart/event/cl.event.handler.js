...
export default class ClEventHandler {
    ...
    bindEvent() {
        if (this.isTouch) {
            ...
            // 实际中用不到 click 会使用鼠标事件来生成
            this.addHandler('dblclick', this.dblclick.bind(this))
    ...
    clearEvent() {
        if (this.isTouch) {
            ...
            this.clearHandler('dblclick', this.dblclick.bind(this))
    ...
    mouseup(event) {
        ...
        //                 this.father.emitEvent('onClick', this.getEventInfo(event))
        //                 this.clickCount = 0;
        //             }, 200); // 300 毫秒是一个常用的双击判定间隔时间
        //         } else {
        //             // 第二次点击，判定为双击事件
        //             clearTimeout(this.clickTimer); // 清除定时器
        //             // console.log('Double click event');
        //             this.father.emitEvent('onDbClick', this.getEventInfo(event))
        //             this.clickCount = 0;
        //         }
        //     } else {
        //         this.clickCount = 0;
        ...
        if (event.touches.length > 1) {
            ...
            if (this.previousTouchPoint) {
                if (Math.abs(this.startX - this.previousTouchPoint.startX) < 10 &&
                    ...
                    this.father.emitEvent('onDbClick', _getTouchInfo(point, event.target))
                }
            }
