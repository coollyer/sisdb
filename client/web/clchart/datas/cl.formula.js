...
import getValue from './cl.data.tools'
...
export class ClFormula {
    ...
    runSingleStock(source, formula) {
        ...
        if (!eval) {
            ...
            for (this.source.nowIndex = this.source.minIndex; this.source.nowIndex <= this.source.maxIndex; this.source.nowIndex++) {
                ...
                singleValue.push([this.getValue('index', 0), out])
            }
            return singleValue
        }
        const singleValue = []
    ...
    SUMMA(label, period) {
        let value = 0
        for (let m = 0; m < period; m++) {
            value += this.getValue(label, m)
        }
        return  value
    }
    AVGPRC() {
        let value = 0
        value = this.getValue('summ', 0) / this.getValue('sumv', 0)
        return value
    }
}
