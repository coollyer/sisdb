    ...
    try {
        ...
        switch (label) {
            case 'index':
                ...
                if (fields.index === undefined) val = index
                else val = source[fields.index]
                break
            default:
...
export function changeSortMode(sortmode) {
    if (sortmode === undefined || sortmode === 'preface') {
        sortmode = 'descending'
...
// 对二维数组中整数列进行升序排序 style = 1 降序 style = 0 升序
export function sortNumberArray(arr, columnIndex, sortmode) {
    if (sortmode === undefined || sortmode === 'ascending') {
        return arr.sort((a, b) => a[columnIndex] - b[columnIndex]);
    } else {
        return arr.sort((a, b) => b[columnIndex] - a[columnIndex]);
    }
}
// 对二维数组中字符串列进行升序排序
export function sortStringArray(arr, columnIndex, sortmode) {
    return arr.sort((a, b) => {
        const valueA = a[columnIndex].toLowerCase();
        const valueB = b[columnIndex].toLowerCase();
        if (valueA < valueB) {
            return (sortmode === undefined || sortmode === 'ascending') ? -1 : 1;
        }
        if (valueA > valueB) {
            return (sortmode === undefined || sortmode === 'ascending') ? 1 : -1;
        }
...
function _transExright (days, rightdata, mode, start, end) {
  ...
  if (mode === 'forword') {
    for (let i = start; i < end; i++) {
      days[i][FIELD_DATE.open] = _getExrightPrice(days[i][FIELD_DATE.open], rightpara, mode) // open
      days[i][FIELD_DATE.high] = _getExrightPrice(days[i][FIELD_DATE.high], rightpara, mode) // high
      days[i][FIELD_DATE.low] = _getExrightPrice(days[i][FIELD_DATE.low], rightpara, mode) // low
      days[i][FIELD_DATE.close] = _getExrightPrice(days[i][FIELD_DATE.close], rightpara, mode) // new
      days[i][FIELD_DATE.vol] = days[i][FIELD_DATE.vol] * rightpara.gs / 1000
    }
  }
}
...
export function transExrightDay (days, rights, mode, start, end) {
  ...
  if (mode === 'forword') {
    for (let i = start; i <= end; i++) {
      for (let j = 0; j < rights.length; j++) {
        if (i < 1) continue
        if (_isRight(days[i - 1][FIELD_DATE.time], days[i][FIELD_DATE.time], rights[j][FIELD_RIGHT.time])) {
  ...
  if (mode === 'forword') {
    for (let i = start; i <= end; i++) {
      for (let j = 0; j < rights.length; j++) {
        if (i < 1) continue
        if (_isRight(
          getDate(days[i - 1][FIELD_DATE.time]),
          getDate(days[i][FIELD_DATE.time]),
 ...
 * @return {Array}
 */
export function checkDayZero (source, fields) {
  const out = []
  if (!Array.isArray(source)) return out

  for (let i = 0; i < source.length; i++) {
    if (!checkZero(source[i], fields)) {
      out.push(source[i])
    }
  }
  return out
...
export function getMinuteCount (tradetime) { // time_t
  ...
  for (let i = 0; i < tradetime.length; i++) {
    mincount += getMinuteGap(tradetime[i][0], tradetime[i][1])
  }
  return mincount
}
 ...
 * @return {Number}
 */
// 需要增加 9 : 30 的信息 - 一共 241 条记录 
export function fromTradeTimeToIndex (ttime, tradetime) { // time_t 返回 0 －240
  ...
  for (let i = 0; i < tradetime.length; i++) {
    ...
    if (minute > tradetime[i][0] && minute <= tradetime[i][1]) {
        index = nowmin + getMinuteGap(tradetime[i][0], minute)
        break
    }
    if (minute <= tradetime[i][0] && i === 0) { // 8:00:00---9:30:59秒前都=0
        return 0
    }
    if (minute <= tradetime[i][0] && (minute > getMinuteOffset(tradetime[i][0], -5))) { // 12:55:59--13:00:59秒
        return nowmin
    }

    nowmin += getMinuteGap(tradetime[i][0], tradetime[i][1])
    if (minute > tradetime[i][1] && i === tradetime.length - 1) { // 15:00:00秒后
        return nowmin
    }
    if (minute > tradetime[i][1] && (minute < getMinuteOffset(tradetime[i][1], 5))) { // 11:30:00--11:34:59秒
        return nowmin
    }
  }
  return index
}
export function getTradeMsecToIndex (msec, tradetime) { // msec 返回 ０－２３９
    return fromTradeTimeToIndex(msec/1000, tradetime)
}
...
export function fromIndexToTradeTime (tindex, tradetime, tradeDate) {
  ...
  for (let i = 0; i < tradetime.length; i++) {
    nowmin = getMinuteGap(tradetime[i][0], tradetime[i][1])
    if (index < nowmin) {
      offset = getMinuteOffset(tradetime[i][0], index + 1)
