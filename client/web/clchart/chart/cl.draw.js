...
export function _fillRect (context, xx, yy, ww, hh, fillclr) {
    ...
    context.fillRect(xx, yy, Math.max(ww, 1), hh)
}
/**
 * clear rect
 * @export
...
export function _drawTxt (context, xx, yy, txt, font, pixel, clr, pos) {
    ...
    // console.log(txt, pos, context.textBaseline);
...
export function _drawKBar (context, config, item) {
  ...
  if (hh === 0) {
    ...
    // console.log(config.index, hh, item[0], item[3], config.unitY);
...
export function _drawVBarZero (context, config, value) {
    const xx = config.rect.left + config.index * (config.unitX + config.spaceX)
    const maxv = Math.abs(config.maxmin.max) + Math.abs(config.maxmin.min) 
    let y0 = config.rect.top + Math.round((config.maxmin.max) * config.unitY)
    let yy = config.rect.top + Math.round((config.maxmin.max - value) * config.unitY)
    let hh = Math.abs(y0 - yy)
    if (yy > y0) {
        yy = y0
    }
    if (config.filled) {
...
export function _drawHBar (context, config, Xvalue, Yvalue) {
    const xx0 = Math.round((config.maxmin.maxv) * config.unitX)
    const xx1 = Math.round((config.maxmin.maxv - Xvalue) * config.unitX)
    
    const yy = config.rect.top + 20 + Math.round((config.maxmin.maxp - Yvalue) * (config.unitY + config.spaceY)) 
    
    // console.log("===", config, xx0, xx1, yy, Xvalue, Yvalue)
    if (Xvalue > 0) {
        _fillRect(context, config.rect.left + xx1, yy, xx0 - xx1, config.spaceY, config.fillclr)
    }
    else {
        _fillRect(context, config.rect.left + xx0, yy, xx1 - xx0, config.spaceY, config.fillclr)
    }
  }
// 以下函数为辅助画图的工具函数
// Adjust 灰度
