...
#include <sisdb_wmsn.h>
#include "sis_utils.h"

// 从行情流文件中获取数据源 需要支持直接写压缩数据然后解压写入

static s_sis_method _sisdb_wmsn_methods[] = {
  {"getcb",  cmd_sisdb_wmsn_getcb, 0, NULL},
};
///////////////////////////////////////////////////
// *** s_sis_modules sis_modules_[dir name]  *** //
///////////////////////////////////////////////////
s_sis_modules sis_modules_sisdb_wmsn = {
    sisdb_wmsn_init,
    NULL,
    NULL,
    NULL,
    ...
    NULL,
    NULL,
    sizeof(_sisdb_wmsn_methods)/sizeof(s_sis_method),
    _sisdb_wmsn_methods,
};

bool sisdb_wmsn_init(void *worker_, void *node_)
{
    s_sis_worker *worker = (s_sis_worker *)worker_; 
    s_sis_json_node *node = (s_sis_json_node *)node_;

    s_sisdb_wmsn_cxt *context = SIS_MALLOC(s_sisdb_wmsn_cxt, context);
...
void sisdb_wmsn_uninit(void *worker_)
{
    s_sis_worker *worker = (s_sis_worker *)worker_; 
    s_sisdb_wmsn_cxt *context = (s_sisdb_wmsn_cxt *)worker->context;

    context->status = SIS_WMAP_EXIT;
    sisdb_wmsn_stop(context);
...
void sisdb_wmsn_stop(s_sisdb_wmsn_cxt *context)
{
    if (context->writer)
    {
...
bool sisdb_wmsn_start(s_sisdb_wmsn_cxt *context)
{
    ...
    if (context->wmode)
    {
        sis_disk_control_remove(sis_sds_save_get(context->work_path), sis_sds_save_get(context->work_name), 
            SIS_DISK_TYPE_MSN, context->work_date > 0 ? context->work_date : -1);
    }
...
int cmd_sisdb_wmsn_getcb(void *worker_, void *argv_)
{
    s_sis_worker *worker = (s_sis_worker *)worker_; 
    s_sisdb_wmsn_cxt *context = (s_sisdb_wmsn_cxt *)worker->context;
